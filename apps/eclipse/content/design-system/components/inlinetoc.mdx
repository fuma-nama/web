...
export default function Page() {
  return <InlineTOC items={tocItems}>On this page</InlineTOC>;
}
```
...
export default function Page({ toc }) {
  ...
  return (
    <>
      <button onClick={() => setIsOpen(!isOpen)}>Toggle TOC</button>

      <InlineTOC items={toc} open={isOpen} onOpenChange={setIsOpen}>
        Table of Contents
      </InlineTOC>
    </>
  );
