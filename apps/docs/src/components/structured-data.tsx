...
type AnyDocsPage = DocsPage | DocsPageV6;
...
function getContentDirectory(page: AnyDocsPage) {
  return page.url.startsWith("/v6") ? "content/docs.v6" : "content/docs";
}

function getSectionTitle(page: AnyDocsPage, slugs: string[]) {
...
function getBreadcrumbName(page: AnyDocsPage, slugs: string[], index: number) {
  ...
  return (
    ...
    slugs[index].charAt(0).toUpperCase() +
      slugs[index].slice(1).replace(/-/g, " ")
  );
}
