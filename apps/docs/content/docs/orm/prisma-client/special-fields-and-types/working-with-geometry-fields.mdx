...
description: "How to read, write, and filter by geometry fields in PostgreSQL"
url: /orm/prisma-client/special-fields-and-types/working-with-geometry-fields
metaTitle: Working with geometry fields (Concepts)
metaDescription: "How to read, write, and filter by geometry fields in PostgreSQL with PostGIS."
---
...
### Reading different geometry types
...
### Setting the value of a geometry field
...
#### Creating a record with a Point
...
#### Creating a record with a LineString
...
#### Creating a record with a Polygon
...
### Setting geometry fields to `null`
...
### Filter by intersection - `intersects`
...
### `NULL` values in geometry filters
...
console.log(
  `Found ${activitiesInPark.length} activities that went through the park`,
);
```
...
### PostgreSQL 16 query planner issue
...
const nearby = await withSpatialOptimization(prisma, (client) =>
...
const zonesContaining = await withSpatialOptimization(prisma, (client) =>
  client.deliveryZone.findMany({
    where: {
      boundary: {
        intersects: {
          type: "Point",
...
- **Server-side filtering**: Use native geometry filters instead of fetching all data and filtering in JavaScript:
  ...
  const nearby = all.filter(
    (loc) => calculateDistance(loc.position, myPoint) < 1000,
  );
...
## Migrating from `Unsupported` type
