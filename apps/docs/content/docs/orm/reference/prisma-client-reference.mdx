...
- `UserWhereUniqueInput` This type works by exposing any unique fields on the model. A field assigned `@id` is considered unique,
  ...
  const whereEmailIsUnique = {
    email: "rich@boop.com",
  } satisfies Prisma.UserWhereUniqueInput;
  ```

- `PostScalarWhereInput`

  ```ts
  const whereScalarTitleIs = {
    title: "boop",
  } satisfies Prisma.PostScalarWhereInput;
  ```
...
- `UserOrderByInput`
  ```ts
  const orderEmailsByDescending = {
    email: "desc",
  } satisfies Prisma.UserOrderByInput;
  ```
