...
url: /v6/orm/prisma-client/client-extensions/model
...
Use the `$extends` [client-level method](/v6/orm/reference/prisma-client-reference#client-methods) to create an _extended client_. An extended client is a variant of the standard Prisma Client that is wrapped by one or more extensions. Use the `model` extension component to add methods to models in your schema.
